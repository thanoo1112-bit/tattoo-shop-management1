...
export function ArtistCustomersClient({ customers }: Props) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Filter Tabs */}
        <div className="flex border-b border-[#262626] md:border-none gap-4">
          {(['all', 'active', 'completed'] as const).map(t => (
            <button
              key={t}
              onClick={() => setFilter(t)}
              className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-[2px] md:mb-0 md:rounded-lg md:border ${
                filter === t
                  ? 'border-[#F5F5F5] text-[#F5F5F5] md:bg-[#F5F5F5] md:text-black md:border-[#F5F5F5]'
                  : 'border-transparent text-[#737373] hover:text-[#A3A3A3] md:border-[#262626] md:hover:bg-[#1F1F1F]'
              }`}
            >
              {t === 'all' && 'ทั้งหมด'}
              {t === 'active' && 'กำลังมีงาน'}
      ...
      {filteredCustomers.length === 0 ? (
        <div className="border border-[#262626] rounded-xl bg-[#171717] p-12 text-center text-[#737373]">
          ...
          <p className="text-xs mt-1 text-[#525252]">ลูกค้าจะปรากฏที่นี่เมื่อมีงานสักกับคุณ</p>
        </div>
      ) : (
                      ...
                      {c.tattoo_projects.map(proj => {
                        ...
                        return (
                          <div key={proj.id} className="bg-[#171717] border border-[#262626] rounded-xl p-5 space-y-4">
                            ...
                            {/* Action to go to My Appointments */}
                            <div className="flex justify-end pt-1">
                              <a
                                href={`/artist/appointments?expandedProjectId=${proj.id}`}
                                className="inline-flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300 transition-colors"
                              >
                                จัดการงานนี้ในนัดหมาย
