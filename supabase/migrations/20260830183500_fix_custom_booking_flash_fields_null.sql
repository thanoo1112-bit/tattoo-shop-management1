-- Migration: Force flash_booking_mode to NULL for custom bookings in finalize_public_booking
...
BEGIN
    ...
    -- For Flash designs, use session mode as fallback if not provided
    IF p_flash_design_id IS NOT NULL THEN
        IF v_mode_val IS NULL THEN
            v_mode_val := v_session.flash_booking_mode;
        END IF;
    ELSE
        -- For custom bookings, flash_booking_mode MUST be NULL
        v_mode_val := NULL;
    END IF;
