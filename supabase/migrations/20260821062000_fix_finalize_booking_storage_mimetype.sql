-- Migration: Fix invalid storage.objects.mimetype column reference in finalize_public_booking
-- Root cause: storage.objects does not contain a top-level mimetype column.
--             The mime value is nested inside the jsonb metadata column (metadata->>'mimetype').
-- Fix: Query metadata->>'mimetype' instead of the nonexistent mimetype column.
...
BEGIN
    ...
    FOREACH v_path IN ARRAY COALESCE(p_real_area_paths, ARRAY[]::text[]) LOOP
        SELECT COALESCE(o.metadata->>'mimetype', 'application/octet-stream') INTO v_meta_mime 
        FROM storage.objects AS o 
        WHERE o.bucket_id = 'tattoo-references' AND o.name = v_path 
        LIMIT 1;
...
-- Preserve execution permissions for public booking roles
