-- Migration: Decouple schedule holds and implement set_flash_booking_review_price RPC
-- File: supabase/migrations/20260830112000_decouple_schedule_holds.sql

-- 1. Redefine booking_schedule_hold_guard to NOT check overlap with other active holds
...
AS $$
BEGIN
    -- Lock artist row for serialization
    PERFORM 1 FROM public.profiles WHERE id = NEW.artist_id FOR UPDATE;

    -- Check overlap with scheduled / in_progress appointments
    IF EXISTS (
    ...
    -- REMOVED: check overlap with other active holds to decouple schedule hold from availability locking

    RETURN NEW;
END;
$$;

-- 2. Redefine appointment_schedule_hold_guard to NOT check overlap with active holds
...
AS $$
BEGIN
    -- No checks needed here; appointments are already protected by native exclude constraint
    RETURN NEW;
END;
$$;

-- 3. Redefine finalize_public_booking to correctly handle non-deposit shops (status = pending_review, no auto-appointment)
CREATE OR REPLACE FUNCTION public.finalize_public_booking(
    ...
    p_flash_booking_mode            text    DEFAULT NULL
) RETURNS uuid
LANGUAGE plpgsql
...
DECLARE
    ...
    v_uid               uuid;
    v_existing          record;
    v_mode_val          text := p_flash_booking_mode;
    v_agreed_price      numeric;
    v_initial_status    text := 'pending_review';
    v_deposit_required  boolean := true;
    v_deposit_amount    numeric := 0;
    v_project_status    text := 'proposed';
BEGIN
    v_uid := auth.uid();
    ...
    -- Use session mode as fallback if not provided
    IF v_mode_val IS NULL THEN
        v_mode_val := v_session.flash_booking_mode;
    END IF;
    ...
    IF p_flash_design_id IS NOT NULL THEN
        ...
        -- Set agreed price based on mode
        IF v_mode_val = 'price_review_required' THEN
            v_agreed_price := NULL;
            v_initial_status := 'pending_review';
            v_project_status := 'proposed';
        ELSE
            v_agreed_price := v_flash_price;
            -- Load shop booking settings to determine deposit
            SELECT deposit_required, default_deposit_amount INTO v_deposit_required, v_deposit_amount
            FROM public.shop_booking_settings
            WHERE shop_id = v_session.shop_id;
            
            IF NOT FOUND THEN
                v_deposit_required := true;
                v_deposit_amount := 500; -- Default fallback
            END IF;

            IF v_deposit_required = false OR v_deposit_amount = 0 THEN
                v_initial_status := 'pending_review';
                v_project_status := 'proposed';
            ELSE
    ...
    IF p_flash_design_id IS NOT NULL AND v_style_name IS NULL THEN
        SELECT style_name INTO v_style_name FROM public.flash_designs WHERE id = p_flash_design_id;
    END IF;
    ...
    INSERT INTO public.tattoo_projects (
        ...
        agreed_price, flash_design_id, flash_variant_id, flash_booking_mode
    )
    VALUES (
        ...
        v_width_val, v_height_val, btrim(p_placement), btrim(p_description), 'Public Flash Booking Request', v_project_status,
        v_agreed_price, p_flash_design_id, p_flash_variant_id, v_mode_val
    )
    ...
    WHILE NOT v_success LOOP
        ...
        BEGIN
            INSERT INTO public.booking_requests (
                ...
                terms_accepted_at, terms_version, tracking_code, flash_design_id, flash_variant_id, flash_booking_mode
            )
            VALUES (
                v_session.shop_id, v_project_id, v_customer_id, v_session.artist_id,
                v_requested_start_at, v_requested_end_at, v_initial_status,
                btrim(p_full_name), p_phone, v_email_val,
                NULLIF(btrim(p_health_note), ''), p_is_first_tattoo, p_safety_notice_acknowledged,
                now(), '2026-08-21-v1', v_tracking_code, p_flash_design_id, p_flash_variant_id, v_mode_val
            )
    ...
    -- If it is Mode A, handle payments/holds
    IF v_mode_val = 'fixed_price' THEN
        IF v_initial_status = 'pending_payment' THEN
            -- Create 24-hour technical hold (pure session marker, does not lock availability)
            INSERT INTO public.booking_schedule_holds (
                shop_id, booking_request_id, artist_id, start_at, end_at, expires_at, created_by
            )
            VALUES (
                v_session.shop_id, v_booking_id, v_session.artist_id, v_requested_start_at, v_requested_end_at, now() + interval '24 hours', NULL
            );

            -- Create pending deposit payment record
            INSERT INTO public.payments (
                shop_id, customer_id, project_id, booking_request_id, payment_type, amount, status
            )
            VALUES (
                v_session.shop_id, v_customer_id, v_project_id, v_booking_id, 'deposit', v_deposit_amount, 'pending'
            );
        END IF;
    END IF;

    -- Clear hold sessions but DO NOT change flash designs status
    IF p_flash_design_id IS NOT NULL THEN
...
GRANT EXECUTE ON FUNCTION public.finalize_public_booking(
    ...
    text[], text[], boolean, boolean, boolean, uuid, uuid, uuid, text
) TO anon, authenticated;

-- 4. Create new RPC set_flash_booking_review_price for Mode B price setting
CREATE OR REPLACE FUNCTION public.set_flash_booking_review_price(
    p_booking_id uuid,
...
BEGIN
    ...
    -- 3. Status and mode checks
    IF v_booking.status != 'pending_review' THEN
        RAISE EXCEPTION 'Booking request is not pending review';
    END IF;
    IF v_booking.flash_booking_mode != 'price_review_required' THEN
        RAISE EXCEPTION 'Booking request is not price_review_required';
    END IF;
    IF p_agreed_price <= 0 THEN
        RAISE EXCEPTION 'Price must be greater than 0';
    END IF;

    -- 4. Assigned artist/owner validation
    IF EXISTS (
        SELECT 1 FROM public.shop_members 
    ...
    ELSIF v_booking.artist_id = v_user_id AND EXISTS (
        SELECT 1 FROM public.shop_members
        WHERE shop_id = v_booking.shop_id
          AND user_id = v_user_id
          AND status = 'active'
    ...
    IF NOT v_is_authorized THEN
        RAISE EXCEPTION 'Unauthorized to set price for this booking';
    END IF;

    -- 5. Update Tattoo Project agreed price
    UPDATE public.tattoo_projects
    SET agreed_price = p_agreed_price,
        updated_at = now()
    WHERE id = v_booking.project_id;

    -- 6. Create technical schedule hold (expires in 24 hours)
    -- This is required by the payment page session, but it is decoupled and does NOT lock the calendar
    INSERT INTO public.booking_schedule_holds (
        shop_id, booking_request_id, artist_id, start_at, end_at, expires_at, created_by
    )
    VALUES (
        v_booking.shop_id, v_booking.id, v_booking.artist_id, v_booking.requested_start_at, v_booking.requested_end_at, now() + interval '24 hours', v_user_id
    );

    -- 7. Create pending deposit payment record
    INSERT INTO public.payments (
        shop_id, customer_id, project_id, booking_request_id, payment_type, amount, status
    )
    VALUES (
        v_booking.shop_id, v_booking.customer_id, v_booking.project_id, v_booking.id, 'deposit', p_deposit_amount, 'pending'
    );

    -- 8. Update booking request status to pending_payment and save accepted audit metadata
...
GRANT EXECUTE ON FUNCTION public.set_flash_booking_review_price(uuid, numeric, numeric) TO authenticated;
