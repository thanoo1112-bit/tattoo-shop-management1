-- Migration: Flash Tattoo Style manual text input addition
-- Modifies flash_designs to store manual style_name and updates booking RPCs to handle nullable style_id.

-- 1. Alter flash_designs table
ALTER TABLE public.flash_designs ALTER COLUMN style_id DROP NOT NULL;
ALTER TABLE public.flash_designs ADD COLUMN IF NOT EXISTS style_name text;

-- 2. Backfill existing records' style_name from tattoo_styles
UPDATE public.flash_designs fd
SET style_name = s.name
FROM public.tattoo_styles s
WHERE fd.style_id = s.id AND fd.style_name IS NULL;

-- 3. Set default / not null constraint on style_name
ALTER TABLE public.flash_designs ALTER COLUMN style_name SET NOT NULL;
ALTER TABLE public.flash_designs DROP CONSTRAINT IF EXISTS flash_designs_style_name_check;
ALTER TABLE public.flash_designs ADD CONSTRAINT flash_designs_style_name_check CHECK (length(trim(style_name)) > 0 AND length(style_name) <= 100);

-- 4. Alter upload sessions table to allow nullable style_id (since Flash can have no style_id)
ALTER TABLE private.public_booking_upload_sessions ALTER COLUMN style_id DROP NOT NULL;

-- 5. Drop old RPC signatures to avoid PostgREST conflicts
...
-- 6. Re-create create_public_booking_upload_session with nullable p_style_id
...
-- 7. Re-create finalize_public_booking to retrieve manual style_name
...
-- 8. Re-declare edit lock trigger to include style_name
