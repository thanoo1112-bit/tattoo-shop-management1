-- Migration: Update RPC functions to support two-mode Flash bookings
-- File: supabase/migrations/20260830110500_update_booking_mode_rpcs.sql

-- 1. Redefine create_public_booking_upload_session to support flash_booking_mode
DROP FUNCTION IF EXISTS public.create_public_booking_upload_session(text, uuid, uuid, text, text, uuid, uuid);
DROP FUNCTION IF EXISTS public.create_public_booking_upload_session(text, uuid, uuid, text, text, uuid, uuid, text);
...
BEGIN
    ...
    INSERT INTO private.public_booking_upload_sessions
        (id, shop_id, artist_id, style_id, color_mode, work_type, expires_at, status, flash_design_id, flash_booking_mode)
    VALUES
        (v_session_id, v_shop_id, p_artist_id, p_style_id, p_color_mode, p_work_type, v_expires_at, 'active', p_flash_design_id, p_flash_booking_mode);

    RETURN QUERY SELECT v_session_id, v_expires_at;
END;
$$;

GRANT EXECUTE ON FUNCTION public.create_public_booking_upload_session(text, uuid, uuid, text, text, uuid, uuid, text) TO anon, authenticated;


-- 2. Redefine finalize_public_booking to support flash_booking_mode and auto-transition to pending_payment for fixed_price
DROP FUNCTION IF EXISTS public.finalize_public_booking(uuid, numeric, numeric, text, text, text, text, text, text, text, text, text[], text[], boolean, boolean, boolean, uuid, uuid, uuid);
DROP FUNCTION IF EXISTS public.finalize_public_booking(uuid, numeric, numeric, text, text, text, text, text, text, text, text, text[], text[], boolean, boolean, boolean, uuid, uuid, uuid, text);
...
BEGIN
    ...
    VALUES (
        ...
        v_width_val, v_height_val, btrim(p_placement), btrim(p_description), 'Public Flash Booking Request', 'proposed',
...
-- 3. Redefine get_public_bookings_by_phone to return flash_booking_mode
...
BEGIN
    ...
    SELECT 
        ...
        s.slug AS shop_slug,
        br.flash_booking_mode AS flash_booking_mode
