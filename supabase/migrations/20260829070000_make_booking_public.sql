-- =============================================================================
-- Migration: Make Booking Public
-- Redefines finalize_public_booking to allow unauthenticated (guest) submissions,
-- resolves customer profiles safely by phone, and grants execute to anon role.
-- Also adds get_public_bookings_by_phone helper for secure phone tracking.
-- =============================================================================
...
BEGIN
    ...
    -- Check if phone exists in this shop
    SELECT id, auth_user_id INTO v_existing
    FROM public.customers
    WHERE shop_id = v_session.shop_id AND phone_normalized = v_phone_norm
    LIMIT 1;
...
-- =============================================================================
-- Helper function: get_public_bookings_by_phone
-- Exposes booking requests query to public guest based on normalized phone,
-- returning only privacy-compliant fields and order by newest first.
-- =============================================================================
...
REVOKE ALL ON FUNCTION public.get_public_bookings_by_phone(text) FROM PUBLIC, anon, authenticated;
