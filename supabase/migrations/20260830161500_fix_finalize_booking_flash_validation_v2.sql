-- Migration: Wrap Flash artist/style mismatch validation to run only for Flash bookings (v2).
-- File: supabase/migrations/20260830161500_fix_finalize_booking_flash_validation_v2.sql
...
BEGIN
    ...
    -- Date Validation: Booking date must be at least tomorrow in Asia/Bangkok
    IF p_requested_date IS NULL OR btrim(p_requested_date) = '' OR p_requested_date::date <= (now() AT TIME ZONE 'Asia/Bangkok')::date THEN
        RAISE EXCEPTION 'กรุณาเลือกวันจองตั้งแต่วันพรุ่งนี้เป็นต้นไป';
    END IF;
    ...
    -- ONLY validate flash matching if this session is for a Flash design
    IF v_session.flash_design_id IS NOT NULL THEN
    ...
    VALUES (
        ...
        v_width_val, v_height_val, btrim(p_placement), btrim(p_description), 'Public Tattoo Booking Request', v_project_status,
    ...
    WHILE NOT v_success LOOP
        ...
        EXCEPTION WHEN unique_violation THEN
            ...
            IF v_retry_count >= v_max_retries THEN
                RAISE EXCEPTION 'Failed to generate unique tracking code after % attempts', v_max_retries;
            END IF;
        END;
    END LOOP;
