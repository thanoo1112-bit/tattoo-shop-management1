-- Migration: Add First Timer and Safety Notice to booking_requests
-- Description: Adds is_first_tattoo and safety_notice_acknowledged boolean fields.
--              Modifies finalize_public_booking to accept these new fields.

ALTER TABLE public.booking_requests 
ADD COLUMN IF NOT EXISTS is_first_tattoo boolean NULL DEFAULT NULL,
ADD COLUMN IF NOT EXISTS safety_notice_acknowledged boolean NULL DEFAULT NULL;

DROP FUNCTION IF EXISTS public.finalize_public_booking(
    uuid, numeric, numeric, text, text, text, text, text, text, text, text, text[], text[], boolean
);
...
BEGIN
    ...
    INSERT INTO public.booking_requests (shop_id, project_id, customer_id, artist_id, requested_start_at, requested_end_at, status, submitted_full_name, submitted_phone, submitted_email, health_note, is_first_tattoo, safety_notice_acknowledged, terms_accepted_at, terms_version)
    VALUES (v_session.shop_id, v_project_id, v_customer_id, v_session.artist_id, v_requested_start_at, v_requested_end_at, 'pending_review', btrim(p_full_name), p_phone, v_email_val, NULLIF(btrim(p_health_note), ''), p_is_first_tattoo, p_safety_notice_acknowledged, now(), '2026-08-21-v1') 
...
REVOKE ALL ON FUNCTION public.finalize_public_booking(uuid, numeric, numeric, text, text, text, text, text, text, text, text, text[], text[], boolean, boolean, boolean) FROM PUBLIC;
