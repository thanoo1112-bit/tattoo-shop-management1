-- =============================================================================
-- Secure Payment Uploads Migration
-- Restricts payment RPCs to the authenticated customer who owns the booking request.
-- Revokes anonymous access.
-- =============================================================================

-- 1. Secure create_public_payment_upload_session
...
BEGIN
    ...
    IF v_uid IS NULL THEN
        RAISE EXCEPTION 'Authentication required to upload payment';
    END IF;
    ...
    -- Ownership verification: resolve customer linked to auth user and match booking
    SELECT id INTO v_customer_id FROM public.customers
    WHERE shop_id = v_booking.shop_id AND auth_user_id = v_uid;

    IF v_customer_id IS DISTINCT FROM v_booking.customer_id THEN
        RAISE EXCEPTION 'Unauthorized to upload payment for this booking';
    END IF;
...
REVOKE ALL ON FUNCTION public.create_public_payment_upload_session(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_public_payment_upload_session(uuid) TO authenticated;

-- 2. Secure submit_public_payment_slip
...
BEGIN
    ...
    v_uid := auth.uid();
    IF v_uid IS NULL THEN
        RAISE EXCEPTION 'Authentication required to submit payment slip';
    END IF;
    ...
    IF v_customer_id IS DISTINCT FROM v_booking.customer_id THEN
        RAISE EXCEPTION 'Unauthorized to submit payment slip for this booking';
    END IF;
...
REVOKE ALL ON FUNCTION public.submit_public_payment_slip(uuid, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.submit_public_payment_slip(uuid, text) TO authenticated;

-- 3. Secure get_public_payment_details
...
BEGIN
    ...
    IF v_customer_id IS DISTINCT FROM v_booking.customer_id THEN
        RAISE EXCEPTION 'Unauthorized to view details for this booking';
    END IF;
...
REVOKE ALL ON FUNCTION public.get_public_payment_details(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_public_payment_details(uuid) TO authenticated;
