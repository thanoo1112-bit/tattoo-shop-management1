-- Drop both existing overload signatures of finalize_public_booking to prevent conflicts
...
DROP FUNCTION IF EXISTS public.finalize_public_booking(uuid, uuid, uuid, text, text, text, text, text, text, text, text, text[], text[], boolean, boolean, boolean, uuid, uuid);
...
CREATE OR REPLACE FUNCTION public.finalize_public_booking(
    ...
    p_flash_design_id               uuid    DEFAULT NULL::uuid,
    p_hold_session_id               uuid    DEFAULT NULL::uuid
) RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
    -- Optional authentication check: If logged in, block back-office staff from creating bookings.
    -- Guests/anon (auth.uid() IS NULL) are fully allowed to create bookings.
    v_current_user := auth.uid();
    IF v_current_user IS NOT NULL THEN
        SELECT EXISTS (
            SELECT 1 FROM public.shop_members 
            WHERE user_id = v_current_user AND status = 'active'
        ) INTO v_is_staff;

        IF v_is_staff THEN
            RAISE EXCEPTION 'บัญชีสำหรับระบบหลังบ้านไม่สามารถสร้างรายการจองลูกค้าได้';
        END IF;
    END IF;
    ...
    -- Set Flash status to reserved immediately (held → reserved)
    IF p_flash_design_id IS NOT NULL THEN
