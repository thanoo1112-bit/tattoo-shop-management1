-- Migration: Short Tracking Code V1
-- Description: Adds tracking_code to public.booking_requests, generates codes for existing bookings,
-- and updates finalize_public_booking to generate unique short tracking codes (XXXX-XXXX-XX) on insert.

-- 1. Create secure generator function
...
-- 2. Add nullable tracking_code column and UNIQUE constraint (before backfill)
ALTER TABLE public.booking_requests ADD COLUMN IF NOT EXISTS tracking_code text;
ALTER TABLE public.booking_requests ADD CONSTRAINT booking_requests_tracking_code_key UNIQUE (tracking_code);
...
DECLARE
  ...
  success boolean;
BEGIN
  FOR r IN SELECT id FROM public.booking_requests WHERE tracking_code IS NULL LOOP
    success := false;
    WHILE NOT success LOOP
      new_code := private.generate_secure_tracking_code();
      BEGIN
...
-- 4. Apply NOT NULL constraint (after successful backfill)
ALTER TABLE public.booking_requests ALTER COLUMN tracking_code SET NOT NULL;

-- 5. Create resolver function for /track page
CREATE OR REPLACE FUNCTION public.resolve_public_booking_tracking_code(
    p_shop_slug text,
...
BEGIN
    ...
    WHERE shop_id = v_shop_id
      AND upper(trim(tracking_code)) = upper(trim(p_tracking_code));

    RETURN v_public_token;
END;
$$;

REVOKE ALL ON FUNCTION public.resolve_public_booking_tracking_code(text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.resolve_public_booking_tracking_code(text, text) TO anon, authenticated;

-- 6. Update finalize_public_booking to generate tracking_code during insertion
CREATE OR REPLACE FUNCTION public.finalize_public_booking(
    ...
    p_is_first_tattoo boolean DEFAULT NULL::boolean,
    p_safety_notice_acknowledged boolean DEFAULT NULL::boolean
)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
    ...
    -- Added variables for short tracking code
...
BEGIN
    ...
    -- Insert booking request with collision loop for tracking_code
    v_success := false;
    WHILE NOT v_success LOOP
        v_tracking_code := private.generate_secure_tracking_code();
        BEGIN
            INSERT INTO public.booking_requests (
                shop_id, project_id, customer_id, artist_id, requested_start_at, requested_end_at, 
                status, submitted_full_name, submitted_phone, submitted_email, health_note, 
                is_first_tattoo, safety_notice_acknowledged, terms_accepted_at, terms_version, tracking_code
            )
            VALUES (
                v_session.shop_id, v_project_id, v_customer_id, v_session.artist_id, v_requested_start_at, v_requested_end_at, 
                'pending_review', btrim(p_full_name), p_phone, v_email_val, NULLIF(btrim(p_health_note), ''), 
                p_is_first_tattoo, p_safety_notice_acknowledged, now(), '2026-08-21-v1', v_tracking_code
            ) 
